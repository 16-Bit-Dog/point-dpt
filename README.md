 # Point-DPT
The initial documented instance of an open-source pointer software that utilizes the position of the user's index finger to ascertain the specific location on the screen they are indicating, without requiring external depth cameras or sensors.
...
With the aid of the index finger's tip location and a point on its body, we establish a direction vector that allows us to ascertain the direction in which the finger is pointing.
### Creating a Depth Map
We use [Intel's dpt-hybrid-midas model](https://huggingface.co/Intel/dpt-hybrid-midas) to generate an estimated depth map for the current frame, and employ this information to determine the Z coordinate of the direction vector.
...
2. The z location of the ray is behind the xy location of the depth image, as depicted in the scenario below.
3. Neither of the above conditions are met, and we proceed to add the X, Y, and Z direction vectors to our current ray position.
...
- Jetson Orion Nano (embedded AI hardware makes this a substantially cheaper option to use) 
or 
- GeForce RTX 3060 

### Future Use-Cases 
Assuming that performance issues are not a significant concern, one possible solution is to send the camera images to an off-shore server to be processed at a lower resolution, thereby reducing the client overhead of using the AI model.
...
- Enables pointing at objects of interest in situations where verbal communication is not possible, facilitating better text-to-speech tools by allowing users to      select specific blocks of text.
